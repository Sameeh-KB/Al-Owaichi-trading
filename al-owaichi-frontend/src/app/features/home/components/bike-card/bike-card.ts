...
@Component({
  ...
  standalone:  true,
  imports:     [RouterLink],
  templateUrl: './bike-card.html',
  styleUrl:    './bike-card.scss',
})
export class BikeCard {
  readonly bike   = input.required<Bike>();
  readonly waPath = WA_PATH;
  ...
  protected readonly waUrl = computed(() =>
    this.langSvc.buildWaUrl(this.catalogSvc.waNumber, this.bike().model)
  );

  protected readonly type = computed(() =>
    this.langSvc.lang() === 'ar' ? this.bike().type_ar : this.bike().type_en
  );

  protected readonly desc = computed(() =>
    this.langSvc.lang() === 'ar' ? this.bike().desc_ar : this.bike().desc_en
  );

  onCardClick(): void {
    this.catalogSvc.track('CARD_CLICK', this.bike().slug);
  }

  onWhatsApp(): void {
    this.catalogSvc.track('WHATSAPP', this.bike().slug);
  }
}
